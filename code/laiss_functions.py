...
def LAISS(
    ...
    print(
        f"REF. : https://alerce.online/object/{ztfid_ref} {tns_name} {tns_cls} {tns_z}"
    )
        ...
        if l_or_ztfid_ref_in_dataset_bank:
            ...
            ref_and_ann_df = pd.DataFrame(
                zip(
                    host_df_ztf_id_l,
                    list(range(0, n + 1)),
                    [tns_name] + tns_ann_names,
                    [tns_cls] + tns_ann_classes,
                    [tns_z] + tns_ann_zs,
                ),
                columns=["ZTFID", "ANN_NUM", "IAU_NAME", "SPEC_CLS", "Z"],
            )
    ...
    if show_lightcurves_grid:
        ...
        if savefigs:
            ...
            plt.savefig(
                f"{figure_path}/{ztfid_ref}_stacked_lightcurve_ann={ann_num}.pdf",
                dpi=300,
                bbox_inches="tight",
            )
            print(
                f"PDF saved at: {figure_path}/{ztfid_ref}_stacked_lightcurve_ann={ann_num}.pdf"
            )
    ...
    if show_hosts_grid:
        ...
        if savetables:
            ...
            host_5ann_df.to_csv(
                f"{table_path}/{ztfid_ref}_host_thumbnails_ann={ann_num}.csv",
                index=False,
            )
            print(
                f"CSV saved at: {table_path}/{ztfid_ref}_host_thumbnails_ann={ann_num}.csv"
            )
